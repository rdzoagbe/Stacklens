...
export function SharedWorkspaceBanner() {
  ...
  const deleteClient = async (org) => {
    ...
    try {
      ...
      await refreshOrgs();
      toast.success((t('ws_deleted_kept'))
    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/60 p-4 pt-16 overflow-y-auto"
      ...
      <div className="w-full max-w-2xl rounded-2xl border border-slate-700 bg-slate-900 shadow-2xl"
        ...
        <div className="flex items-start justify-between gap-4 border-b border-slate-800 px-5 py-4">
          <div>
            <h2 className="text-base font-semibold text-white">{t('ws_manage_title')}</h2>
        ...
        <div className="px-5 py-4 space-y-5">
          <section>
            ...
            {orgs.length === 0 ? (
              <p className="text-sm text-slate-500">{t('ws_none_live')}</p>
            ) : (
